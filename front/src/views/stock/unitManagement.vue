  ...
  <el-card class="cardClass">
    ...
    <el-collapse-transition>
      <el-form :model="addForm" :inline="true" :rules="rules" ref="addForm" label-width="80px" v-if="hidden">
        <el-form-item label="货物单位" prop="unitName">
          <el-input v-model="addForm.unitName" placeholder="请输入货物单位" size="small" clearable></el-input>
        </el-form-item>
        <el-form-item label="单位备注" prop="unitNotes">
          <el-input v-model="addForm.unitNotes" placeholder="请输入货物单位备注" size="small" clearable></el-input>
        </el-form-item>
        <el-form-item>
    ...
    <el-table :data="tableData" size="small" border stripe style="width: 100%;" v-loading="loading">
      ...
      <el-table-column prop="unitName" label="货物单位" min-width="30px" align="center">
      </el-table-column>
      <el-table-column prop="unitNotes" label="备注" align="center" :show-overflow-tooltip="true"></el-table-column>
      <el-table-column width="300px" align="center">
        <template slot="header" slot-scope="scope">
          <el-form :model="ruleForm" :inline="true" :rules="rules" ref="ruleForm">
            <div style="display:flex;justify-content: center;">
              <el-input v-model="ruleForm.value" size="mini" style="margin-right:10px;" placeholder="输入货物单位搜索"
                clearable />
        ...
        <template slot-scope="scope">
          <!-- 修改单位开始 -->
          <el-popover :title="'修改单位：' + scope.row.unitName" placement="bottom" width="270"
            :ref="'pop' + scope.$index">
            <el-form :model="updateForm" :rules="rules" ref="updateForm" label-width="80px" class="popoverClass">
              <el-form-item label="单位名称" prop="unitName" :show-message="false">
                <el-input v-model="updateForm.unitName" placeholder="请输入货物单位" size="mini" clearable></el-input>
              </el-form-item>
              <el-form-item label="单位备注" prop="unitNotes">
                <el-input v-model="updateForm.unitNotes" placeholder="请输入货物单位备注" size="mini" clearable></el-input>
              </el-form-item>
...
<script>
import { queryUnit, saveUnit, updateUnit, deleteUnit } from '@/apis/stock/unitManagement.js'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        unitName: [{ required: true, message: '请输入新增单位', trigger: 'blur' }],
      },
      tableData: null,
    }
  },
  ...
  methods: {
    ...
    addSubmit(formName) {
      ...
      _this.$refs[formName].validate((valid) => {
        if (valid) {
          saveUnit(_this.addForm).then(function (res) {
            _this.search();
            if (res.code !== 0) {
              type = 'warning'
            }
            _this.$message({
              message: '货物单位：“' + _this.addForm.unitName + '”' + res.msg,
              type: type
            });
    ...
    del(row) {
      const _this = this;
      _this.$confirm('确认删除单位“' + row.unitName + '”吗？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteUnit(row).then(function (res) {
          // console.log(res);
          if (res.code === 0) {
            _this.$message({
              type: 'success',
              message: '单位“' + row.unitName + "”" + res.msg,
            })
            _this.search();
    ...
    search() {
      const _this = this;
      _this.loading = true
      queryUnit(_this.ruleForm).then(function (res) {
    ...
    delItemData() {
      this.$refs['updateForm'].validate((valid) => {
        if (valid) {
          const _this = this;
          updateUnit(_this.updateForm).then(function (res) {
            // console.log("res: ", res.code);
            if (res.code === 0) {
              _this.$alert(_this.updateForm.unitName + res.msg, '提示', {
                confirmButtonText: '确定',
                callback: action => {
                  _this.search()
                }
